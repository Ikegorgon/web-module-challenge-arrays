...
newFlavors = copy(originalFlavors);
...
console.log(getFlavorByIndex(newFlavors, 2));
...
console.log(removeFlavorByName(newFlavors, "Rocky Road"));
console.log(newFlavors.length);
...
console.log(filterByWord(newFlavors, "Chocolate"));
...
function getAverageWordLength(arr){
  let wordLen = 0;
  for (item in arr) {
    wordLen += arr[item].split(" ").length;
  }
  wordLen= wordLen / arr.length;
  return wordLen;
}
console.log(getAverageWordLength(originalFlavors));
console.log(getAverageWordLength(filterByWord(newFlavors, "Chocolate")));
...
function getRandomFlavors(originalArr, currentArr, seasonalArr, regionalArr){
  let randomFlavors = [];
  randomFlavors = randomFlavors.concat(originalArr, currentArr, seasonalArr, regionalArr);
  for (let i = 0; i < randomFlavors.length; i++) {
    let j = Math.floor(Math.random() * (i+1));
    let temp = randomFlavors[i];
    randomFlavors[i] = randomFlavors[j];
    randomFlavors[j] = temp;
  }
  randomFlavors.length = 31;
...
console.log(getRandomFlavors(originalFlavors, currentFlavors, seasonalFlavors, regionalFlavors));
